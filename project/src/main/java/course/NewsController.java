...
/**
 * This class encapsulates the controllers for the news web application.  It delegates all interaction with MongoDB
...
public class NewsController {
    ...
    public static void main(String[] args) throws IOException {
        if (args.length == 0) {
            new NewsController("mongodb://localhost");
        }
        else {
            new NewsController(args[0]);
        }
    }

    public NewsController(String mongoURIString) throws IOException {
        final MongoClient mongoClient = new MongoClient(new MongoClientURI(mongoURIString));
        final MongoDatabase newsDatabase = mongoClient.getDatabase("news");
        newsPostDAO = new NewsPostDAO(newsDatabase);
        userDAO = new UserDAO(newsDatabase);
        sessionDAO = new SessionDAO(newsDatabase);
    ...
    abstract class FreemarkerBasedRoute extends Route {
        ...
        /**
         *
         * @param path The route path which is used for matching. (e.g. /Hey, users/:name)
         */
    ...
    private void initializeRoutes() throws IOException {
        // This is the news landing page
        get(new FreemarkerBasedRoute("/", "news_template.ftl") {
            @Override
            public void doHandle(Request request, Response response, Writer writer) throws IOException, TemplateException {
                String username = sessionDAO.findUserNameBySessionId(getSessionCookie(request));
                List<Document> posts = newsPostDAO.findByDateDescending(10);
                List<Document> hotwords = newsPostDAO.findByHotWords(10);
                List<Document> publisher = newsPostDAO.findByPublisher(10);
                List<Document> commenter = newsPostDAO.findByCommenter(10);
                SimpleHash root = new SimpleHash();
                root.put("myposts", posts);
                root.put("myhotwords", hotwords);
                root.put("mostpublisher", publisher);
                root.put("mostcommenter", commenter);
                if (username != null) {
        ...
        // used to display actual details of a news post
        get(new FreemarkerBasedRoute("/post/:permalink", "entry_template.ftl") {
            ...
            protected void doHandle(Request request, Response response, Writer writer) throws IOException, TemplateException {
                ...
                String username = sessionDAO.findUserNameBySessionId(getSessionCookie(request));
                Document post = newsPostDAO.findByPermalink(permalink);
                if (post == null) {
        ...
        // used to delete details of a news post
        get(new FreemarkerBasedRoute("/post/delete/:permalink", "entry_template.ftl") {
            @Override
            protected void doHandle(Request request, Response response, Writer writer) throws IOException, TemplateException {
                String permalink = request.params(":permalink");
                boolean deletepost = newsPostDAO.deleteByPermalink(permalink);
				if (deletepost == false) {
				   response.redirect("/post_not_found");
				}
				else {
                    // error
                    SimpleHash root = new SimpleHash();
                    root.put("deletepost", deletepost);
                    template.process(root, writer);
                }
           }
        });
            ...
            protected void doHandle(Request request, Response response, Writer writer) throws IOException, TemplateException {
                ...
                if (validateSignup(username, password, verify, email, root)) {
                    ...
                    if (!userDAO.addUser(username, password, email)) {
                        ...
                        // all okay, let's start a session
        ...
        // will present the form used to process new news posts
        get(new FreemarkerBasedRoute("/newpost", "newpost_template.ftl") {
            ...
            protected void doHandle(Request request, Response response, Writer writer) throws IOException, TemplateException {
                ...
                String username = sessionDAO.findUserNameBySessionId(getSessionCookie(request));
                if (username == null) {
                    // bad request. user is not logged in
        ...
        post(new FreemarkerBasedRoute("/newpost", "newpost_template.ftl") {
            ...
            protected void doHandle(Request request, Response response, Writer writer)
                ...
                String username = sessionDAO.findUserNameBySessionId(getSessionCookie(request));
                if (username == null) {
                    response.redirect("/login");    // only logged in users can post to news
                }
                else if (title.equals("") || post.equals("")) {
                    // redisplay page with errors
                    HashMap<String, String> root = new HashMap<String, String>();
                    root.put("errors", "post must contain a title and news entry.");
                    ...
                    String permalink = newsPostDAO.addPost(title, post, tagsArray, username);
                    // now redirect to the news permalink
        ...
        get(new FreemarkerBasedRoute("/welcome", "welcome.ftl") {
            ...
            protected void doHandle(Request request, Response response, Writer writer) throws IOException, TemplateException {
                ...
                if (username == null) {
                    ...
                    List<Document> commenter = newsPostDAO.findByCommenter(10);
                    SimpleHash root = new SimpleHash();
                    List<Document> posts = newsPostDAO.findByUserNameDescending(username);
                    root.put("myposts", posts);
                    root.put("username", username);
                    root.put("myhotwords", hotwords);
                    root.put("mostpublisher", publisher);
        ...
        post(new FreemarkerBasedRoute("/newcomment", "entry_template.ftl") {
            ...
            protected void doHandle(Request request, Response response, Writer writer)
                ...
                // check that comment is good
                    ...
                    newsPostDAO.addPostComment(name, email, body, permalink);
                    response.redirect("/post/" + permalink);
                }
            }
        });
        ...
        get(new FreemarkerBasedRoute("/tag/:thetag", "news_template.ftl") {
            @Override
            protected void doHandle(Request request, Response response, Writer writer)
                    throws IOException, TemplateException {
                String username = sessionDAO.findUserNameBySessionId(getSessionCookie(request));
                ...
                List<Document> posts = newsPostDAO.findByTagDateDescending(tag);
                root.put("myposts", posts);
                root.put("myhotwords", hotwords);
                root.put("mostpublisher", publisher);
                root.put("mostcommenter", commenter);
        ...
        // allows the user to logout of the news
    ...
    private Configuration createFreemarkerConfiguration() {
        ...
        retVal.setClassForTemplateLoading(NewsController.class, "/freemarker");
        return retVal;
    }
}
